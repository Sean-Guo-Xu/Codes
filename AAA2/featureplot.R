...
load("E:\\AAA_scRNA\\all_AAA.Rdata")
pbmc=subset(pbmc,cell!="Unsure")
myfeatureplot(pbmc,allgene, "#6BB7CA",  "#E07B54","Artery A")
load("E:\\AAA_scRNA\\6Normal.Rdata")
myfeatureplot(pbmc,allgene, "#6BB7CA",  "#E07B54","Artery N")

load("E:\\AAA_scRNA\\Blood\\all_AAA.Rdata")
myfeatureplot(pbmc,allgene, "#6BB7CA",  "#E07B54","Blood A")
load("E:\\AAA_scRNA\\Blood\\N_blood.Rdata")
myfeatureplot(pbmc,allgene, "#6BB7CA",  "#E07B54","Blood N")
...
pbmc = subset(pbmc,cell != "Unsure")
for (i in allgene) {
VlnPlot(pbmc,features = i,group.by = "cell",raster = F,split.by ="sample",cols = c("#E07B54","#6BB7CA"))+stat_compare_means( aes(label = ..p.signif..),  method = "wilcox.test")+xlab("")
...
testAB.anchors <- FindIntegrationAnchors(object.list = list(aaa,pbmc), dims = 1:20)
...
Monocyte = c("FCN1","APOBEC3A","THBS1") #2 5 8 9 19 22http://127.0.0.1:21567/graphics/plot_zoom_png?width=1692&height=407
...
Granulocytes = c("S100A8","CSF3R","TREM1","IL1R2")
...
cluster = c(rep("T cell",3),rep("B cell",3),rep("NK",3),rep("Mast cell",3)
            ,rep("Plasma",3),rep("Monocyte",3),rep("Megakaryocyte",3),rep("DC",3),rep("Neutrophils",4))
...
ggsave("Blood_A_dotplot.tiff",width = 14,height = 5)
...
pbmc$cell[pbmc$cell %in% "Granulocytes"] = "Neutrophil"
...
top10heatmap <- function(pbmc, name) {
pbmc = subset(pbmc,cell != "Unsure")
pbmc@active.ident = as.factor(pbmc$cell)
pbmc = ScaleData(pbmc)
...
library(ggplot2)
library(ggsci)
DoHeatmap(pbmc,features = top10$gene, group.colors =pal_d3()(length(unique(pbmc$cell))),size = 2,angle = 90)+scale_fill_gradient(low="#6BB7CA",high="#E07B54")
ggsave(paste(name,"top10 heatmap"),width = 8, height = 12)
}
load("F:\\AAA_scRNA\\all_AAA.Rdata")
top10heatmap(pbmc,"Artery_A")
load("F:\\AAA_scRNA\\6Normal.Rdata")
top10heatmap(pbmc,"Artery_N")
load("F:\\AAA_scRNA\\all_AAA.Rdata")
myumapplot(pbmc,"cell","Artery_A")
load("E:\\AAA_scRNA\\Blood\\A_blood.Rdata")
myumapplot(pbmc,"cell","Blood_A")
load("E:\\AAA_scRNA\\Blood\\N_blood.Rdata")
DoHeatmap(pbmc,features = marker )
