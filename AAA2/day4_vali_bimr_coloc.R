...
#2）rs编号`rs_id`、P值`pval_nominal`、case在所有样本中的比例`s`，MAF也要，写在list最�?

#如果表型是连续型变量，输入文件三选一�?
#1）rs编号`rs_id`、P值`pval_nominal`、表型的标准差`sdY`�?
#2）rs编号`rs_id`、P值`pval_nominal`、效应值`beta`,效应值方�? `varbeta`, 样本量`N`,次等位基因频�? `MAF`�?
...
#MAFs代表次要等位基因的频率，因此其范围在0�?0.5之间。值大�?0.5的所有EAF通过�?1中减去它们而转化为MAF�?
