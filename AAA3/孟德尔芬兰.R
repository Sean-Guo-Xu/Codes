...
save(gwas,file="Europe_gwas.Rdata")

load("Europe_gwas.Rdata")
gwas = gwas[gwas$V5 != "",]
write.table(cbind(gwas$V5,gwas$V7),"snpp",row.names = F,col.names = F,sep="\t",quote=F)
...
#gene = c("HLA-DRA","HLA-DPA1","HLA-DPB1","HLA-DQA1","HLA-DMA","HLA-DMB","HLA-DQB1","HLA-DRB5")
...
write.csv(pheno, "HLA_confounder.result.csv", row.names=F)
...
write.table(mr_res,"PBMC_MR_res.txt",row.names = F,col.names = T,sep = "\t",quote = F)
...
ggsave("volcano.tiff",width = 5.5,height = 5)
...
write.table(table1,"MR_SNP_res.txt",row.names = F,col.names = T,sep = "\t")
