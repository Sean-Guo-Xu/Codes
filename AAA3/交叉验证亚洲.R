...
setwd("E:\\AAA_gwas")
or = read.table("OR.txt",sep = "\t",header = T)
...
write.table(result_or[,4:ncol(result_or)],"Asian_test_OR.txt",row.names = F,sep = "\t",quote = F)
...
dev.off()
write.csv(pleio_dat,"Asian_test_pleiotropy_test.csv",row.names = F)
write.csv(heter_dat,"Asian_test_heterogeneity_test.csv",row.names = F)
write.csv(direction,"Asian_test_direction_test.csv",row.names = F)
write.csv(steiger,"Asian_test_steiger_test.csv",row.names = F)
#########????????
library(TwoSampleMR)
bimr_SLE = data.frame(
  pos = gwas$POS,
...
write.table(allresult[,4:ncol(allresult)],"Asian_test_reverse_bi_OR.txt",row.names = F,sep = "\t",quote = F)

mydata=read.table("Asian_test_reverse_bi_OR.txt",header = T,sep = "\t")
...
library(grid) 
library(forestploter)
pdf("Asian_test_reverse_mr.pdf",width = 10,height = 12)
mydata$exposure = "AAA"
...
mydata$`OR (95% CI)` <- ifelse(is.na(mydata$or), "",sprintf("%.4f (%.4f - %.4f)",
                                                            mydata$or, mydata$or_lci95, 
                                                            mydata$or_uci95))
pdf("Asian_test_final forest.pdf",width = 12,height = 12)
colnames(mydata)[c(1:3,6)]= c("Exposure","Method","nSNP","Pval")
forest(data=mydata[,c(1:3,6,13,14)],
       est = mydata$or,
       lower =mydata$or_lci95, 
