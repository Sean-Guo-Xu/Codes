...
save(A_cellchat,N_cellchat,cellchat,chatlist,file="E:\\AAA_scRNA\\mouse_myeloid_cellchat.Rdata")
load("E:\\AAA_scRNA\\mouse_myeloid_cellchat.Rdata")
setwd("E:\\AAA_scRNA\\cellchat\\mouse")
col = rep(c("#f57245","#e6f49a","#A6CEE3","#fbe18c","#90d1a4","#33A02C","#FB9A99"),1)
names(col) =  c("Mono","Foamy-Mø","TRM-Mø","Inf-Mø","cDC1","cDC2","IFNIC-Mø")            
...
for (i in 1:length(chatlist)) {
  gg[[i]] <- netAnalysis_signalingRole_scatter(chatlist[[i]], title = names(chatlist)[i], weight.MinMax = weight.MinMax)+scale_x_continuous(limits = c(0, 0.4))+scale_y_continuous(limits = c(0, 0.32))+scale_color_manual(values = col)
}
