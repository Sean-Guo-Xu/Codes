...
load("mouse_normal_all.Rdata")

pbmc[["percent.mt"]] <- PercentageFeatureSet(object = pbmc, pattern = "^mt-")
VlnPlot(pbmc,features = c("percent.mt","nFeature_RNA"))
pbmc <- PercentageFeatureSet(pbmc, "^Hb[^(P)]", col.name = "percent_hb")
pbmc <- subset(x = pbmc, subset = nFeature_RNA > 200& nFeature_RNA <6500 & percent.mt < 25 & percent_hb<5)
...
DimPlot(pbmc,label = T,group.by = "DF.classifications_0.25_0.3_2142")
pbmc = subset(pbmc,DF.classifications_0.25_0.3_2142== "Singlet" )
DimPlot(pbmc,label = T,group.by = "DF.classifications_0.25_0.3_2142")
save(pbmc,file = "mouse_normal_all.Rdata")
...
save(pbmc.markers,file="N_mouse_marker.Rdata")
load("N_mouse_marker.Rdata")
load("N_mouse_marker.Rdata")
nk <- c("Klra8","Klrb1c","Klrc1")
t <- c("Cd3e","Cd4","Cd3g")
mast <- c("Mrgprb1","Cpa3","Kit")
ec<- c("Cdh5","Pecam1","Tie1")
fb <- c("Pdgfra","Col1a1","Lum")
smc <- c("Acta2","Myh11","Cnn1")
myeloid <- c("Adgre1","Cd68","Cd14")
b <- c("Cd79a","Cd79b","Ly6d")
granulocyte <- c("S100a8","S100a9","Camp")
Proliferation = c("Mki67","Top2a","Cdc20") 
#14 unsure
markers = c(nk,t,ec,fb,smc ,myeloid,b,granulocyte,Proliferation)
############?ֶ?ע??#########

DotPlot(pbmc,features = markers)+RotatedAxis()
pbmc$cell = pbmc$orig.ident
pbmc$cell[pbmc$seurat_clusters %in% c(19,13)] = "NK/T cell"
pbmc$cell[pbmc$seurat_clusters %in% c(16)] = "Endothelial"
pbmc$cell[pbmc$seurat_clusters %in% c(18,15,9,5,4,8,3)] = "Fibroblast"
pbmc$cell[pbmc$seurat_clusters %in% c(0,14,2,7,1,11,10)] = "SMC"
pbmc$cell[pbmc$seurat_clusters %in% c(6,17)]="Myeloid"
pbmc$cell[pbmc$seurat_clusters %in% c(21)] = "B cell"
pbmc  = subset(pbmc,seurat_clusters != 12)
pbmc  = subset(pbmc,seurat_clusters != 20)
DotPlot(pbmc,features = markers,group.by = "cell")+RotatedAxis()
DimPlot(pbmc,group.by = c("cell"))
save(pbmc,file= "mouse_normal_all.Rdata")
pbmc=subset(pbmc,cell == "Myeloid")
save(pbmc,file = "mouse_normal_myeloid.Rdata")
