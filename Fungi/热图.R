...
withclass = read.table("D:\\bigslice\\bgc_withclass.txt",sep="\t",header=F,fill=T)
...
dt = dt[,c(1,4,7,2,5,6,3)]
mypal2 = mypal2[c(1,4,7,2,5,6,3)]
bar1 <- HeatmapAnnotation(
  sum1 = anno_barplot(
    ...
    border = F,
    axis_param = list(at = c(0,25000,50000,75000,100000),
...
bar3 = rowAnnotation(
  sum3 = anno_barplot(
    ...
    axis_param = list(at = c(0,1,2,3,4),
                      labels = c("0","10",expression("10" ^ 2),expression("10" ^ 3),expression("10" ^ 4)),
                      labels_rot=45
    ),
...
bar5 = rowAnnotation(
  sum5 = anno_barplot(
    ...
    width = unit(2, "cm")), show_annotation_name = F)
dt2=dt2[,c(1,4,7,2,5,6,3)]
pdf("heatmap.pdf",width = 6.5,height = 5)
...
write.table(dt2,"heat_table.xls",row.names = T,col.names = T,sep = "\t")
write.table(dt,"bar_table.xls",row.names = T,col.names = T,sep = "\t")
