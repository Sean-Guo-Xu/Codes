...
small=classify[which(classify$V7 %in% target2 ),]
cutphy=small[1,3]
class = read.table("D:\\bigslice\\bgc_information.txt",sep="\t",header=F,fill=T)
gcf=list()
g=c(0)
for (i in name[-which(name$Name %in% cutphy),1] ){
...
write.table(mat,paste(target2,"chord.txt"),sep="\t",quote = F)
tiff("D:\\bigslice\\Aspergillus&Fusarium&otherphy.tiff",width = 3000,height = 3000,units = "px")
